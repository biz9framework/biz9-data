                        ...
                        function get_data(search_item) {
                            return new Promise((resolve) => {
							    ...
							    let search = Data_Logic.get_search(Type.DATA_GROUP,group_query,{},search_item.page_current,search_item.page_size);
 								Portal.search_simple(database,cache,search.data_type,search.filter,search.sort_by,search.page_current,search.page_size,item_group_join_option).then(([biz_error,biz_data])=>{
                                    if(biz_error){
                                        error=Log.append(error,biz_error);
                                    }else{
                        ...
                        for(const sub_search_item of foreign_search_foreign_items){
							for(const data_item of data_items){
								...
								for(const sub_data_item of data_item[search_item.title]){
									console.log('start');
									console.log('start');
                           			cool = await Portal.get_data_foreign_search(database,cache,data_item[search_item.title],sub_search_item);
                           			//data_item[search_item.title] = await Portal.get_data_foreign_search(database,cache,data_item[search_item.title],sub_search_item);
									console.log('ssss');
									console.log(cool);
									console.log('tttt');
								}
