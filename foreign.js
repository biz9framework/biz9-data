    ...
    static get_search_item_data =(database,cache,search_item)=> {
        return new Promise((callback) => {
            ...
            async.series([
                ...
                async function(call){
                    //Log.w('33_my_search_item',search_item);
                    //for(const sub_search_item of search_item.foreigns){
                        //Log.w('44_sub_search_item',sub_search_item);
                        //Foreign.get_search_item_detail_data(database,cache,sub_search_item).then((error,sub_search_item) => {
                            //sub_search_item = sub_search_item;
                            //call();
                        //});
                    //}
                }
                    ...
                    let sub_foreign_search_items = [];
                        if(search_item.value_type != Data_Value_Type.COUNT){
                            for(const sub_search_item of search_item.foreigns){
                                ...
                                sub_search_foreign_item = await Foreign.run_search_item_data(database,cache,sub_search_foreign_item);
                                for(const data_item of search_item.items){
